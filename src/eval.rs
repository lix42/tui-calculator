...
/// A cursor over a token slice, mirroring `Parser` but indexing `&[Token]`
/// instead of `Vec<char>`. Tokens are `Copy`, so `peek`/`advance` hand back
/// owned values.
...
/// Token-based entry point: evaluate a committed expression to an `f64`.
/// Same grammar as `eval`, but consumes `&[Token]` directly so the caller
/// never has to format computed values back into a string.
pub fn eval_tokens(tokens: &[Token]) -> Result<f64, String> {
    let mut c = TokenCursor::new(tokens);
    let value = parse_expr_tokens(&mut c)?;
...
/// expr = term (('+' | '-') term)*
fn parse_expr_tokens(c: &mut TokenCursor) -> Result<f64, String> {
    let mut lhs = parse_term_tokens(c)?;
    loop {
        match c.peek() {
            Some(Token::Op('+')) => {
                c.advance();
                lhs += parse_term_tokens(c)?;
            }
            Some(Token::Op('-')) => {
                c.advance();
                lhs -= parse_term_tokens(c)?;
...
/// term = factor (('*' | '/') factor)*
fn parse_term_tokens(c: &mut TokenCursor) -> Result<f64, String> {
    let mut lhs = parse_factor_tokens(c)?;
    loop {
        match c.peek() {
            Some(Token::Op('*')) => {
                c.advance();
                lhs *= parse_factor_tokens(c)?;
            }
            Some(Token::Op('/')) => {
                c.advance();
                let rhs = parse_factor_tokens(c)?;
                if rhs == 0.0 {
                    return Err("division by zero".to_string());
                }
                lhs /= rhs;
...
/// factor = '-' factor | '(' expr ')' | number
fn parse_factor_tokens(c: &mut TokenCursor) -> Result<f64, String> {
    match c.peek() {
        Some(Token::Op('-')) => {
            c.advance();
            Ok(-parse_factor_tokens(c)?)
        }
        Some(Token::LParen) => {
            c.advance();
            let value = parse_expr_tokens(c)?;
            match c.advance() {
